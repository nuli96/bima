...
<blockquote><b>perintah : <code>{0}didin gantenk ga</code> - <code>{0}tes on</code></blockquote></b>
...
async def _(client, message):
    ...
    deleted_users = []
    banned_users = 0
    Tm = await message.reply("<blockquote><b>sᴇᴅᴀɴɢ ᴍᴇᴍᴇʀɪᴋsᴀ</blockquote></b>")
    ...
    if len(deleted_users) > 0:
        for deleted_user in deleted_users:
            try:
                ...
                pass
        await Tm.edit(f"<blockquote><b>{brhsl} ʙᴇʀʜᴀsɪʟ ᴍᴇɴɢᴇʟᴜᴀʀᴋᴀɴ {banned_users} ᴀᴋᴜɴ ᴛᴇʀʜᴀᴘᴜs ᴅɪ ɢʀᴏᴜᴘ : {message.chat.title}</blockquote></b>")
    else:
        await Tm.edit(f"<blockquote><b>{ggl} ᴛɪᴅᴀᴋ ᴀᴅᴀ ᴀᴋᴜɴ ᴛᴇʀʜᴀᴘᴜs ᴅɪ ɢʀᴏᴜᴘ ɪɴɪ</blockquote></b>")
...
{tion} ᴜsᴇʀʙᴏᴛ ᴏɴ ʙᴀɴɢ ᴅɪɴ</blockquote>

<blockquote><b>-- USERBOT 15K/BULAN BY @USERBOTDIN --</b></blockquote>
"""
            await message.reply(_ping)
        else:
            await message.reply(f"<blockquote>ᴘᴏɴɢ : {str(delta_ping_formatted).replace('.', ',')} ms\nᴜsᴇʀʙᴏᴛ ᴏɴ ʙᴀɴɢ ᴅɪɴ</blockquote>\n\n<blockquote><b>USERBOT 15K/BULAN BY @USERBOTDIN</b></blockquote>")
...
@PY.INDRI("didin gantenk ga")
async def didingantenkga(client, message):
    await message.reply(
       "<blockquote><b>ya benar dia sangat gantenk sekali\n\n- dia baik\n- dia manis\n- dia lucu\n- dia imut\n- dia konbrut awsjshsjhsjs\n\nidaman banget lah pokonya didin nih</blockquote></b>")

@PY.INDRI("tes on")
async def teson(client, message):
    await message.reply(
       "<blockquote><b>on selalu didin gantenk</blockquote></b>")

@PY.INDRI("sedih bgt rek tele utama ilang")
async def sedih(client, message):
    await message.reply(
        "<b>kenapa didin sedih ? semangat ya kan ada kita kita disini 😁<b>")

@PY.INDRI("didin sedih")
async def sedih(client, message):
    await message.reply(
        "<b>kenapa didin sedih ? semangat ya kan ada kita kita disini 😁<b>")
